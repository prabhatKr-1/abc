...
func RaiseBookRequest(c *gin.Context) {
	...
	if err := config.DB.Where("lib_id = ? AND reader_Id= ? AND book_ID = ? AND Request_Type = ?", libId, id, input.ISBN, input.RequestType).First(&bookReq).Error; err == nil {
		c.JSON(http.StatusBadRequest, gin.H{
...
func MyRequests(c *gin.Context) {
	...
	if err := config.DB.Where("reader_id = ? AND lib_id = ?", id, libId).Find(&reqs).Error; err != nil {
		c.JSON(http.StatusOK, gin.H{
			"message": err.Error(),
		})
		return
